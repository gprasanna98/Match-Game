...
class MatchGame extends Component {
  ...
  setActiveThumbnailId = imageUrl => {
    this.setState({
      activeThumbnailId: imageUrl,
    })
  }

  getfilteredImages = () => {
    const {activeTabId} = this.state
    const searchResults = imagesList.filter(
  ...
  timer = () => {
    ...
    if (initialValue > 1) {
      this.setState(prevState => ({initialValue: prevState.initialValue - 1}))
  ...
  renderMatchImage = () => {
    ...
    this.setState({
      activeThumbnailId: randomNumber,
    })
  }

  render() {
    const {activeTabId, activeThumbnailId, initialValue, score} = this.state
      ...
      <div className="app-container">
        <div className="header-container">
          ...
          <div className="timer-container">
            <p className="score">
              Score:<span className="score-points">{score}</span>
            </p>
            <img
            ...
            />
            <p className="score-points">{initialValue} secs</p>
          </div>
        </div>
          ...
          <ul className="images-list">
            {filteredImages.map(eachImage => (
              <ImageItem
                ...
                active={eachImage.id === activeThumbnailId}
                setActiveThumbnailId={this.renderMatchImage}
              />
            ))}
          </ul>
        </div>
